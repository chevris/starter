...
class PresetsComponent extends Component {
	...
	render() {
		...
		return (
			<div className="themeslug-presets">
				<div className="search">
					<input
						type="search"
						placeholder={ __( 'Search Presets…', 'themeslug' ) }
						onChange={ ( e ) => {
				...
				{ 0 < presets.length ? (
					...
					<p>{ __( 'No presets found', 'themeslug' ) }</p>
				) }
			</div>
		);
	}
	...
	replaceSettings( setup ) {
		...
		Object.keys( setup ).map( ( themeMod ) => {
			...
			document.dispatchEvent(
				new CustomEvent( 'themeslug-changed-customizer-value', {
					detail: {
