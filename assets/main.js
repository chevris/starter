...
function initModals() {
  micromodal__WEBPACK_IMPORTED_MODULE_0__["default"].init({
    ...
    disableScroll: false,
    // handled by the modalCallback()
    awaitOpenAnimation: false,
    // if true, wait end of open animation to focus on an element in the modal.
    awaitCloseAnimation: true,
    // if true, wait end of close animation before removing modal from DOM
...
function modalCallback(modalEl) {
  ...
  var scrollBarWidth = window.innerWidth - htmlElement.clientWidth;
  var triggerButton = document.querySelector("button[data-micromodal-trigger=\"".concat(modalEl.id, "\"]"));
  var closeButton = modalEl.querySelector('button[data-micromodal-close]'); // Use aria-hidden to determine the status of the modal, as this attribute is
  // managed by micromodal.

  var isHidden = 'true' === modalEl.getAttribute('aria-hidden');
  triggerButton.setAttribute('aria-expanded', !isHidden);
  closeButton.setAttribute('aria-expanded', !isHidden); // Add a class to indicate a modal is open.
